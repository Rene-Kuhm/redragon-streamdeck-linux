...
fn cpu_temp_millidegrees() -> Option<i32> {
    cpu_temp_millidegrees_in(std::path::Path::new("/sys/class/hwmon"))
}

fn cpu_temp_millidegrees_in(hwmon_root: &std::path::Path) -> Option<i32> {
    ...
    CPU_HWMON_NAMES.iter().find_map(|wanted| {
        sensors
            ...
            .find(|(name, _)| {
                name == wanted || name.starts_with(&format!("{wanted}_"))
            })
...
mod tests {
    ...
    fn reports_nothing_when_no_known_cpu_sensor_exists() {
        ...
        let root = fake_hwmon("no-cpu-sensor", &[("nvme", "43850"), ("r8169_0_600:00", "38500")]);
    ...
    fn ydotool_socket_candidates_prefer_the_runtime_dir() {
        ...
        assert_eq!(candidates[0], PathBuf::from("/run/user/1000/.ydotool_socket"));
